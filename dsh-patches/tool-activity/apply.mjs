...
const CARD_COMPONENT = block(2, [
  ...
  '*',
  '* `skill` and the nested question go LAST, after every field above. Skill',
  '* sends `{skill, args}` and AskUserQuestion sends `{questions: [...]}`, so',
  '* neither shares a key with anything earlier and their position carries no',
  '* risk — but both were absent from this chain entirely, which is why those',
  '* two cards rendered a BLANK summary: no skill name, no question text.',
  '*',
  '* AskUserQuestion is the one tool whose summary is not a top-level string, so',
  '* it is dug out defensively: a non-array `questions`, an empty one, or an',
  '* entry carrying no string `question` must degrade to "" rather than throw.',
  '* A throw inside this helper takes down the whole assistant message tree.',
  '*/',
  'function toolActivitySummary(activity) {',
  T + 'const input = activity.input !== null && typeof activity.input === "object" ? activity.input : {};',
  T + 'const asked = Array.isArray(input.questions) ? input.questions[0] : void 0;',
  T + 'const question = asked !== null && typeof asked === "object" && typeof asked.question === "string" ? asked.question : void 0;',
  T + 'const value = input.description ?? input.file_path ?? input.command ?? input.pattern ?? input.path ?? input.query ?? input.url ?? input.skill ?? question ?? "";',
